...
{
    uint32_t u32Src;
    const uint32_t au32Clk[] = {(16000UL), (32768UL), 0, 0, (49152000UL)};

    if(timer == ((TMR_T *) ((( uint32_t)0x40000000) + 0x10000)))
        u32Src = (((CLK_T *) ((( uint32_t)0x50000000) + 0x00200))->CLKSEL1 & (0x7ul << (8))) >> (8);
    else if(timer == ((TMR_T *) ((( uint32_t)0x40000000) + 0x10020)))
...
uint32_t TIMER_GetWorkingFreq(TMR_T *timer)
{
    return TIMER_GetModuleClock(timer)/((timer->CMP)*((timer->CTL&(0xfful << (0)))+1));
}
