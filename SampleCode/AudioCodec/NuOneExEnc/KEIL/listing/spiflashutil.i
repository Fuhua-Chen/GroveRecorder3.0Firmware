...
#line 2 "..\\..\\..\\..\\Library\\Framework\\src\\SPIFlashUtil.c"
	...
	PFN_SPIFLASHUTIL_WRITE_CALLBACK pfnSpiFlashWriteCallbck )
{
	...
	if ( u32Start4kPageAddr%4096 )
	{
		
		SPIFlash_Erase4K(&g_sSpiFlash,u32Start4kPageAddr/4096, 1);
	}
}
...
BOOL SPIFlashUtil_4KPagePartialWriting(void)
{
	...
	if ( SPIFlash_CheckBusy(&g_sSpiFlash) == 1 )
		return 1;

	
	
	if ( g_pfnSpiFlashWriteCallbck!=0 )
		g_pfnSpiFlashWriteCallbck();
	
	if ( g_u16PartialWriteDataLength == 0 )
		return 0;

	if ((g_u32SPIFlashRecAddr%(4096) == 0)	
		&&(u8SPiFlashCtrl != 1))		
	{
...
{	
	...
	while( SPIFlashUtil_4KPagePartialWriting() == 1 );
}
...
void SPIFlashUtil_WriteEncodeData(S_BUF_CTRL *psEncodeBufCtrl)
{		
	if ( SPIFlashUtil_4KPagePartialWriting() == 0 )
	{
