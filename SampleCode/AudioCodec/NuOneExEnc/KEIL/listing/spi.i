...
void SPI_Close(SPI_T *spi)
{
    ...
    if((uint32_t)spi == ((( uint32_t)0x40000000) + 0x30000)) 
	{
...
uint32_t SPI_SetBusClock(SPI_T *spi, uint32_t u32BusClock)
{
    ...
    if(u32BusClock !=0 ) 
	{
        ...
        if(u32Div > 0xFFFF)
			...
			u32Div = 1;
        spi->CLKDIV = (spi->CLKDIV & ~(0xfffful << (0))) | u32Div;
    } 
	else
        spi->CLKDIV = 0;
...
void SPI_SetVarClock(SPI_T *spi, uint32_t u32VarClock)
{
    ...
    if(u32VarClock !=0 ) 
	{
        ...
        if(u32Div > 0xFFFF)
			...
			u32Div = 1;
        spi->CLKDIV = (spi->CLKDIV & ~(0xfffful << (16))) | (u32Div<<(16));
    } else
        spi->CLKDIV = 0;
}
...
uint32_t SPI_GetVarClock(SPI_T *spi)
{
	...
	u32ApbClock = CLK_GetHCLKFreq();
	u32Div = (spi->CLKDIV&(0xfffful << (16)))>>(16);
	return ((u32ApbClock>>1) / (u32Div+1));	
}
