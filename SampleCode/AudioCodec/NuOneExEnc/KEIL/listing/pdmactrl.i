...
#line 2 "..\\..\\..\\..\\Library\\Framework\\src\\PdmaCtrl.c"
...
void PdmaCtrl_Init(void)
{
	...
	CLK_EnableModuleClock(((0UL<<30)|(0<<28)|(0x0<<25)|( 0<<20)|(0<<18)|(0x0<<10)|( 0<<5)|(1)));
...
)
{
	...
	PDMA_T *pdma = (PDMA_T *)((uint32_t) ((( uint32_t)0x50000000) + 0x08000) + (0x100 * u32Ch));
	volatile int32_t i = 10;
	
	
	PDMA_Open( 1<<u32Ch );	
	
	pdma->DSCT_CTL |= (0x1ul << (1));
	while(i--);                                  
	...
	PDMA_SetTransferAddr( u32Ch, 0, (u32ServiceSel&(0x3ul << (4))), 0, (u32ServiceSel&(0x3ul << (6))));
	
	PDMA_SetTransferCnt( u32Ch, (u32ServiceSel&(0x3ul << (19))), 0 );
	
	PDMA_SetTransferDirection( u32Ch, (u32ServiceSel&(0x3ul << (2))) );	
	
	pdma->DSCT_CTL |= (u32ServiceSel&(0xful << (12)));
	
	
	(*((volatile uint32_t *)((uint32_t)&((PDMA_T *) ((( uint32_t)0x50000000) + 0x08000))->CHIF + (uint32_t)((u32Ch)*0x100))) = (0x00000503));
...
{
	(*((volatile uint32_t *)((uint32_t)&((PDMA_T *) ((( uint32_t)0x50000000) + 0x08000))->DSCT_ENDSA + (uint32_t)((u32Ch)*0x100))) = ((uint32_t)p32SrcAdd)); 
	(*((volatile uint32_t *)((uint32_t)&((PDMA_T *) ((( uint32_t)0x50000000) + 0x08000))->DSCT_ENDDA + (uint32_t)((u32Ch)*0x100))) = ((uint32_t)p32DesAdd));
	PDMA_SetTransferCnt( u32Ch, 0x00100000UL, u16Count );
	PDMA_Trigger(u32Ch);
}

void PdmaCtrl_Stop(UINT32 u32Ch)
{
	(*((volatile uint32_t *)((uint32_t)&((PDMA_T *) ((( uint32_t)0x50000000) + 0x08000))->DSCT_CTL + (uint32_t)((u32Ch)*0x100))) &= ~(0x1ul << (0)));
}

void PdmaCtrl_Close(UINT32 u32Ch)
{
	(*((volatile uint32_t *)((uint32_t)&((PDMA_T *) ((( uint32_t)0x50000000) + 0x08000))->DSCT_CTL + (uint32_t)((u32Ch)*0x100))) &= ~(0x1ul << (0)));
	((PDMA_GCR_T *) ((( uint32_t)0x50000000) + 0x08F00))->GLOCTL &= ~((1<<u32Ch)<<8);
}
...
void PdmaCtrl_UnInit(void)
{
	...
	CLK_DisableModuleClock(((0UL<<30)|(0<<28)|(0x0<<25)|( 0<<20)|(0<<18)|(0x0<<10)|( 0<<5)|(1)));
}
